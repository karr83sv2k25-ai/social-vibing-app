...
export default function CoinPurchaseScreen({navigation}) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(userRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        const newCoins = userData.wallet?.coins || 0;

        // Trigger pulse animation if balance increased
        if (newCoins > userCoins && !loadingBalance) {
    ...
    return () => unsubscribe();
  }, [userCoins, loadingBalance]);

  // Handle purchase
  const handlePurchase = (productId) => {
    Alert.alert(
      'Confirm Purchase',
      `Purchase ${IAP_PRODUCTS.COINS[productId].amount} coins?`,
      [
  ...
  // Get coin products from config
  const coinProducts = Object.entries(IAP_PRODUCTS.COINS).map(([id, product]) => {
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
              ...
              {loadingBalance ? (
                ...
                <PulseView trigger={balanceUpdated}>
                  <View style={styles.balanceRow}>
                    <Ionicons name="cash" size={24} color={GOLD} />
            ...
            {coinProducts.map((product, index) => {
              ...
              return (
                <FadeInView key={product.id} delay={index * 100} style={{width: '47%'}}>
                  ...
                  <Ionicons name="cash" size={48} color={GOLD} />
        ...
        <FadeInView delay={600}>
          <View style={styles.infoSection}>
            <Text style={styles.infoTitle}>💡 About Coins</Text>
            <Text style={styles.infoText}>
              • Use coins to unlock premium marketplace items{'\n'}
              • Purchase stickers, comics, books, and more{'\n'}
